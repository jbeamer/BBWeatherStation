 ...
 * update the webpage display
 ...
 February, 2023
 0.4.0:  OTA refactor to ArduinoOTA
 ...
 July 26, 2021 
 0.2.1:  working, but super janky hardware set up
 **************************************************************************/
char *str_version = "version 0.4.0";
...
void DisplayLine(const char *strText, bool clearScreen, int row) {
...
void DisplayHeader(const char *strHeader) {
  DisplayLine(strHeader, true, 1);
}

void handleServerCall(void) {
  ...
  html_page.concat("\n </p>\n <p><a href='https://thingspeak.com/channels/265281/private_show'>ThingSpeak Page</a></p>");
...
void loop() {
  ...
  // this variable is set in the interrupt and processed here, although this
  // is so minor that it could be moved to interrupt handler:
  if (g_bNewAnemometerTick) {
  ...
  // read button on front of box, which will determine if display is on
